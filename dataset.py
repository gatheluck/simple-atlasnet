...
class ShapeNet(data.Dataset):
	"""
	- rootimg: root dir of rendered img.
	- rootpc: root dir of point cloud data.
	- class_choice: list of using class. None means all.
	- train: if true, return data for training.
	- npoints: number of sampling points.
	- normal: (return with normals)
	- balanced: not used here... 
	"""
	def __init__(self, 
							rootimg = os.path.join('data','ShapeNet','ShapeNetRendering'), 
							rootpc = os.path.join('data','customShapeNet'),
							class_choice = "chair", 
							train = True, 
							npoints = 2500, 
							normal = False, 
		...
		self.cat = {} # {plane:02691156, ..., watercraft:04530566}
		self.meta = {}
		self.SVR = SVR
		self.gen_view = gen_view
		self.idx=idx
		# read labels and dirnames from self.catfile.
		...
		for item in self.cat:
			...
			try:
				...
				fns_pc = []
			fns = [val for val in fns_img if val + '.points.ply' in fns_pc] # select valid training data
			print('category ', self.cat[item], 'files ' + str(len(fns)), len(fns)/float(len(fns_img)), "%"),
			
			# train use first 80% data, test use last 20% data
			if train:
